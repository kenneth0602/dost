...
const handler = (event, callback) => {
  console.log('Update Scholarship Handler')
  ...
  try {
    pool.getConnection((err, connection) => {
        ...
        // return event.file;
        const query = `CALL scholarship_update(${id}, "${title}", "${category}", "${sponsor}","${participation_fee}","${venue}");`;
        connection.query(query, (err, result) => {
            if (err) {
                console.error('Error Updating into Scholarship table:', err);
