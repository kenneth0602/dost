...
const handler = (event,callback) => {
    ...
    const query = `call divChief_getAllPendingRequestFromAdminByDivID(${divID},${pageNo},${pageSize}, '${keyword}' );`;
    ...
    pool.getConnection((err,connection) => {
        ...
        connection.query(`SELECT empID FROM users WHERE username ='${username}'`, (err, result) => {
            ...
            connection.query(`INSERT INTO audit_logs(username, target, action) VALUES('${username}', 'All Pending Request from Admin', 'View');`, (err1, result1) => {
                if(err1) {
                    console.log(err1);
