...
const handler = (event,callback) => {
    ...
    const query = `call SME_updateSMEbyID(${profileID},'${lastname}', '${firstname}', '${middlename}', '${telNo}', '${mobileNo}', '${companyName}', '${companyAddress}', '${companyNo}', '${emailAdd}', '${fbMessenger}', '${viberAccount}', '${website}', '${areaOfExpertise}', '${affiliation}' ,'${resource}', ${honorariaRate}, '${TIN}');`;

    // console.log(viberAccount);
    ...
    pool.getConnection((err,connection) => {
        ...
        connection.query(`select empID from admin where username ='${username}'`,(err,result)=>{
            ...
            connection.query(`insert into audit_logs(username, target, action) values('${username}','Subject Matter Expert','Modify');`,(err1,result1)=>{
        ...
        connection.query(query, (error,results,fields) => {
            ...
            if(error) {
                ...
                callback.status(200).send({message : 'Successfully updated SME Profile'});
            }
        });
    })
});
