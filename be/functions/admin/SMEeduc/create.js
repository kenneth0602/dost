...
const handler = (event,callback) => {
    ...
    pool.getConnection((err,connection) => {
        ...
        connection.query(`select empID from admin where username ='${username}'`,(err,result)=>{
            ...
            connection.query(`insert into audit_logs(username, target, action) values('${username}','SME Educational Background','Create');`,(err1,result1)=>{
        ...
        connection.query(query, (error,results,fields) => {
            ...
            if(error) {
                ...
                callback.status(200).send({message : 'Successfully created new SME Background'});
            }
        });
    })
});
