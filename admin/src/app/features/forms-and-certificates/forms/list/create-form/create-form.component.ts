...
export class CreateFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const fieldGroup = this.addField(); // ✅ capture returned fieldGroup
    this.handleFieldTypeValidation(fieldGroup); // ✅ now you can pass it
  }
  ...
  handleFieldTypeValidation(fieldGroup: FormGroup) {
    ...
    typeControl?.valueChanges.subscribe((type: string) => {
      ...
      if (type === 'Radio' || type === 'Checkbox') {
        // Add 2 empty options for choice-based fields
        optionsArray.push(this.createOption());
        optionsArray.push(this.createOption());

        // Correct answer required for choices
        ...
        // Textbox: no correct answer or options required
      }
  ...
  submitForm(): void {
    ...
    this.featuresService.createForms(payload, jwt).subscribe({
      next: () => {
